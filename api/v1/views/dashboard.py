...
def dashboard(usrId):
    ...
    res["budgets"] = []
    allbdgts = [bdgt for bdgt in storage.all(
        Budget).values() if bdgt.user_id == usrId]
    last = [bdgt for bdgt in allbdgts if bdgt.confirmed ==
            True and datetime.utcnow() - bdgt.created_at <= timedelta(days=3)]
    for bdgt in last:
        bdgts = {
            "id": bdgt.id,
    ...
    res["active_budgets"] = len(
        [bdgt for bdgt in allbdgts if bdgt.confirmed == True])
